...
Phase 4 step 20 corrected final-tree expectations without changing any query text. The baseline predicted `EditorNotifications.cxx` and `EditorNotificationsTest.cxx` for `SetModEventMask`, but step 12 deliberately placed the fixed-host notification policy in `EditorHost.cxx` with `EditorHostTest.cxx`; the step 19 run therefore assigned no rank even when `EditorHost.cxx` appeared. Undo command-name queries continue to target `case EditorCommand::Undo` in `EditorCommands.cxx`, while its intent, effect, and held-out queries target the named `Editor::Undo` operation in `EditorHistory.cxx`. LineDown command-name queries continue to target `EditorCommands.cxx`, while its behavior queries target `Editor::CursorUpOrDown`, one of the shared movement helpers that the step 18 audit deliberately retained in `Editor.cxx`. The command test filename was corrected from the never-created `EditorCommandsTest.cxx` to `EditorCommandTest.cxx`. The deleted `SetTechnology` rows now name the retained-entry audit, which verifies that deleted callables have no dispatch cases, instead of the never-created `EditorDrawingTest.cxx`. These are corrections to recorded ownership and evidence, not changes made in response to search ranks; the uncorrected results remain in `benchmark-results/phase4-step19/`.

Phase 5 step 10 (2026-07-16) left every query string and target concern unchanged. It stripped deleted message-layer paths (`ScintillaDoc.html`, `Scintilla.iface`, `ScintillaMessages.h`, and related client headers) from `obsolete_locations`, and pointed `SetTechnology` evidence at `MESSAGE_REMOVAL.md` after `tools/check-retained-entrypoints.sh` was removed in step 6. Query text was not edited to chase ranks.

Pilot measures are under [`benchmark-results/wrapping-pilot/`](benchmark-results/wrapping-pilot/observations.md) and [`benchmark-results/autocomplete-calltip-pilot/`](benchmark-results/autocomplete-calltip-pilot/observations.md). Phase 4 step 19 recorded a post-concern-move cleanup measure under [`benchmark-results/phase4-step19/`](benchmark-results/phase4-step19/observations.md) without changing the fixed corpus; that write-up notes the gaps taken into the step 20 gate. The corrected Phase 4 matrix, boundary check, cold navigation, exact-name checks, and completion audit are under [`benchmark-results/phase4-final/`](benchmark-results/phase4-final/observations.md). Phase 5 reran the unchanged corpus after deleting the generated message and client layers; its matrix, cold navigation, exact-name checks, and removal audit are under [`benchmark-results/phase5-final/`](benchmark-results/phase5-final/observations.md).
