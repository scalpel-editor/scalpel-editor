...
#include <OnlyWayUi/Core/DecorationTypes.h>
#include <OnlyWayUi/Core/Dictionary.h>
#include <OnlyWayUi/Core/FileInterface.h>
...
#include <OnlyWayUi/Core/NumericValue.h>
#include <OnlyWayUi/Core/Platform.h>
#include <OnlyWayUi/Core/Rectangle.h>
#include <OnlyWayUi/Core/RenderTypes.h>
#include <OnlyWayUi/Core/Span.h>
#include <OnlyWayUi/Core/StringUtilities.h>
#include <OnlyWayUi/Core/SystemInterface.h>
#include <OnlyWayUi/Core/Unit.h>
#include <OnlyWayUi/Core/Variant.h>
#include <OnlyWayUi/Core/Vector2.h>
#include <OnlyWayUi/Core/Vector3.h>
#include <OnlyWayUi/Core/Vector4.h>
#include <OnlyWayUi/Core/Vertex.h>
#include <stdint.h>
#include <stdio.h>
#include <algorithm>
#include <string.h>
...
#define OWUI_SHADER_HEADER_VERSION "#version 330\n"
#define OWUI_GL3_LOADER_IMPLEMENTATION
#include "OnlyWayUi_Include_GL3.h"
...
#ifndef OWUI_NUM_MSAA_SAMPLES
	#define OWUI_NUM_MSAA_SAMPLES 2
#endif
...
#define OWUI_STRINGIFY_IMPL(x) #x
#define OWUI_STRINGIFY(x) OWUI_STRINGIFY_IMPL(x)

#define OWUI_SHADER_HEADER \
	OWUI_SHADER_HEADER_VERSION "#define MAX_NUM_STOPS " OWUI_STRINGIFY(MAX_NUM_STOPS) "\n#line " OWUI_STRINGIFY(__LINE__) "\n"

static const char* shader_vert_main = OWUI_SHADER_HEADER R"(
...
static const char* shader_frag_texture = OWUI_SHADER_HEADER R"(
...
static const char* shader_frag_color = OWUI_SHADER_HEADER R"(
in vec2 fragTexCoord;
in vec4 fragColor;
...
static const char* shader_frag_gradient = OWUI_SHADER_HEADER R"(
#define LINEAR 0
#define RADIAL 1
#define CONIC 2
...
static const char* shader_frag_creation = OWUI_SHADER_HEADER R"(
...
static const char* shader_vert_passthrough = OWUI_SHADER_HEADER R"(
in vec2 inPosition;
in vec2 inTexCoord0;
...
static const char* shader_frag_passthrough = OWUI_SHADER_HEADER R"(
uniform sampler2D _tex;
in vec2 fragTexCoord;
out vec4 finalColor;
...
static const char* shader_frag_color_matrix = OWUI_SHADER_HEADER R"(
uniform sampler2D _tex;
...
static const char* shader_frag_blend_mask = OWUI_SHADER_HEADER R"(
uniform sampler2D _tex;
...
#define OWUI_SHADER_BLUR_HEADER \
	OWUI_SHADER_HEADER "\n#define BLUR_SIZE " OWUI_STRINGIFY(BLUR_SIZE) "\n#define BLUR_NUM_WEIGHTS " OWUI_STRINGIFY(BLUR_NUM_WEIGHTS)

static const char* shader_vert_blur = OWUI_SHADER_BLUR_HEADER R"(
...
static const char* shader_frag_blur = OWUI_SHADER_BLUR_HEADER R"(
uniform sampler2D _tex;
...
static const char* shader_frag_drop_shadow = OWUI_SHADER_HEADER R"(
uniform sampler2D _tex;
uniform vec2 _texCoordMin;
uniform vec2 _texCoordMax;
uniform vec4 _color;
...
public:
	...
	{
		OWUI_ASSERT((size_t)id < (size_t)Enum::Count);
...
private:
	OnlyWayUi::Array<T, (size_t)Enum::Count> ids = {};
};
...
private:
	...
	OnlyWayUi::UnorderedMap<Key, GLint> map;
};
...
static void CheckGLError(const char* operation_name)
{
	...
	{
		static const OnlyWayUi::Pair<GLenum, const char*> error_names[] = {{GL_INVALID_ENUM, "GL_INVALID_ENUM"}, {GL_INVALID_VALUE, "GL_INVALID_VALUE"},
		...
		OnlyWayUi::Log::Message(OnlyWayUi::Log::LT_ERROR, "OpenGL error during %s. Error code 0x%x (%s).", operation_name, error_code, error_str);
	}
#endif
	(void)operation_name;
}
...
{
	OWUI_ASSERT(shader_type == GL_VERTEX_SHADER || shader_type == GL_FRAGMENT_SHADER);
	...
	if (status == GL_FALSE)
	{
		...
		OnlyWayUi::Log::Message(OnlyWayUi::Log::LT_ERROR, "Compile failure in OpenGL shader: %s", info_log_string);
...
static bool CreateProgram(GLuint& out_program, Uniforms& inout_uniform_map, ProgramId program_id, GLuint vertex_shader, GLuint fragment_shader)
{
	...
	if (status == GL_FALSE)
	{
		...
		OnlyWayUi::Log::Message(OnlyWayUi::Log::LT_ERROR, "OpenGL program linking failure: %s", info_log_string);
		delete[] info_log_string;
		glDeleteProgram(id);
		return false;
	}
	...
	for (int unif = 0; unif < num_active_uniforms; ++unif)
	{
		...
		{
			OnlyWayUi::Log::Message(OnlyWayUi::Log::LT_ERROR, "OpenGL program uses unknown uniform '%s'.", name_buf);
			return false;
		}
	}
	...
	GLuint shared_depth_stencil_buffer)
{
	...
	const OnlyWayUi::Colourf border_color(0.f, 0.f);
	...
	{
		OnlyWayUi::Log::Message(OnlyWayUi::Log::LT_ERROR, "OpenGL framebuffer could not be generated. Error code %x.", framebuffer_status);
		return false;
	}
...
static GLuint CreateTexture(OnlyWayUi::Span<const OnlyWayUi::byte> source_data, OnlyWayUi::Vector2i source_dimensions)
{
	GLuint texture_id = 0;
...
static void BindTexture(const FramebufferData& fb)
{
	if (!fb.color_tex_buffer)
	{
		OWUI_ERRORMSG("Only framebuffers with color textures can be bound as textures. This framebuffer probably uses multisampling which needs a "
					   "blit step first.");
	}
...
{
	OWUI_ASSERT(std::all_of(data.vert_shaders.begin(), data.vert_shaders.end(), [](auto&& value) { return value == 0; }));
	OWUI_ASSERT(std::all_of(data.frag_shaders.begin(), data.frag_shaders.end(), [](auto&& value) { return value == 0; }));
	OWUI_ASSERT(std::all_of(data.programs.begin(), data.programs.end(), [](auto&& value) { return value == 0; }));
	auto ReportError = [](const char* type, const char* name) {
		OnlyWayUi::Log::Message(OnlyWayUi::Log::LT_ERROR, "Could not create OpenGL %s: '%s'.", type, name);
		return false;
	};
...
{
	auto mut_program_data = OnlyWayUi::MakeUnique<Gfx::ProgramData>();
	if (Gfx::CreateShaders(*mut_program_data))
	{
		program_data = std::move(mut_program_data);
		OnlyWayUi::Mesh mesh;
		OnlyWayUi::MeshUtilities::GenerateQuad(mesh, OnlyWayUi::Vector2f(-1), OnlyWayUi::Vector2f(2), {});
...
{
	viewport_width = OnlyWayUi::Math::Max(width, 1);
	viewport_height = OnlyWayUi::Math::Max(height, 1);
	viewport_offset_x = offset_x;
	viewport_offset_y = offset_y;
	projection = OnlyWayUi::Matrix4f::ProjectOrtho(0, (float)viewport_width, (float)viewport_height, 0, -10000, 10000);
}

void RenderInterface_GL3::BeginFrame()
{
	BeginFrame(OnlyWayUi::Rectanglei::FromSize({viewport_width, viewport_height}));
}

void RenderInterface_GL3::BeginFrame(OnlyWayUi::Rectanglei damage_region)
{
	OWUI_ASSERT(viewport_width >= 1 && viewport_height >= 1);
	const OnlyWayUi::Rectanglei viewport_region = OnlyWayUi::Rectanglei::FromSize({viewport_width, viewport_height});
	frame_damage_region = damage_region.Valid() ? damage_region.Intersect(viewport_region) : viewport_region;
	if (frame_damage_region.Width() <= 0 || frame_damage_region.Height() <= 0)
		frame_damage_region = viewport_region;
	...
	scissor_state = OnlyWayUi::Rectanglei::MakeInvalid();
	SetScissor(frame_damage_region);
...
OnlyWayUi::CompiledGeometryHandle RenderInterface_GL3::CompileGeometry(OnlyWayUi::Span<const OnlyWayUi::Vertex> vertices, OnlyWayUi::Span<const int> indices)
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(OnlyWayUi::Vertex) * vertices.size(), (const void*)vertices.data(), draw_usage);

	glEnableVertexAttribArray((GLuint)Gfx::VertexAttribute::Position);
	glVertexAttribPointer((GLuint)Gfx::VertexAttribute::Position, 2, GL_FLOAT, GL_FALSE, sizeof(OnlyWayUi::Vertex),
		(const GLvoid*)(offsetof(OnlyWayUi::Vertex, position)));

	glEnableVertexAttribArray((GLuint)Gfx::VertexAttribute::Color0);
	glVertexAttribPointer((GLuint)Gfx::VertexAttribute::Color0, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(OnlyWayUi::Vertex),
		(const GLvoid*)(offsetof(OnlyWayUi::Vertex, colour)));

	glEnableVertexAttribArray((GLuint)Gfx::VertexAttribute::TexCoord0);
	glVertexAttribPointer((GLuint)Gfx::VertexAttribute::TexCoord0, 2, GL_FLOAT, GL_FALSE, sizeof(OnlyWayUi::Vertex),
		(const GLvoid*)(offsetof(OnlyWayUi::Vertex, tex_coord)));
	...
	return (OnlyWayUi::CompiledGeometryHandle)geometry;
}

void RenderInterface_GL3::RenderGeometry(OnlyWayUi::CompiledGeometryHandle handle, OnlyWayUi::Vector2f translation, OnlyWayUi::TextureHandle texture)
{
...
void RenderInterface_GL3::ReleaseGeometry(OnlyWayUi::CompiledGeometryHandle handle)
{
	Gfx::CompiledGeometryData* geometry = (Gfx::CompiledGeometryData*)handle;
...
/// @note Changes the coordinate system from OnlyWayUi to OpenGL, or equivalently in reverse.
/// @note The Rectangle::Top and Rectangle::Bottom members will have reverse meaning in the returned rectangle.
static OnlyWayUi::Rectanglei VerticallyFlipped(OnlyWayUi::Rectanglei rect, int viewport_height)
{
	OWUI_ASSERT(rect.Valid());
	OnlyWayUi::Rectanglei flipped_rect = rect;
	flipped_rect.p0.y = viewport_height - rect.p1.y;
	flipped_rect.p1.y = viewport_height - rect.p0.y;
	return flipped_rect;
}

void RenderInterface_GL3::SetScissor(OnlyWayUi::Rectanglei region, bool vertically_flip)
{
	if (!vertically_flip && frame_damage_region.Valid())
		region = region.Valid() ? region.Intersect(frame_damage_region) : frame_damage_region;
	...
	if (region.Valid() && region != scissor_state)
	{
		...
		const int x = OnlyWayUi::Math::Clamp(region.Left(), 0, viewport_width);
		const int y = OnlyWayUi::Math::Clamp(viewport_height - region.Bottom(), 0, viewport_height);
...
void RenderInterface_GL3::EnableScissorRegion(bool enable)
{
	...
	if (!enable)
		SetScissor(OnlyWayUi::Rectanglei::MakeInvalid(), false);
}

void RenderInterface_GL3::SetScissorRegion(OnlyWayUi::Rectanglei region)
{
	SetScissor(region);
}
...
void RenderInterface_GL3::RenderToClipMask(OnlyWayUi::ClipMaskOperation operation, OnlyWayUi::CompiledGeometryHandle geometry, OnlyWayUi::Vector2f translation)
{
	OWUI_ASSERT(glIsEnabled(GL_STENCIL_TEST));
	using OnlyWayUi::ClipMaskOperation;
...
OnlyWayUi::TextureHandle RenderInterface_GL3::LoadTexture(OnlyWayUi::Vector2i& texture_dimensions, const OnlyWayUi::String& source)
{
	OnlyWayUi::FileInterface* file_interface = OnlyWayUi::GetFileInterface();
	OnlyWayUi::FileHandle file_handle = file_interface->Open(source);
	if (!file_handle)
	{
		return false;
	}
	...
	{
		OnlyWayUi::Log::Message(OnlyWayUi::Log::LT_ERROR, "Texture file size is smaller than TGAHeader, file is not a valid TGA image.");
		file_interface->Close(file_handle);
		return false;
	}

	using OnlyWayUi::byte;
	OnlyWayUi::UniquePtr<byte[]> buffer(new byte[buffer_size]);
	...
	{
		OnlyWayUi::Log::Message(OnlyWayUi::Log::LT_ERROR, "Only 24/32bit uncompressed TGAs are supported.");
		return false;
	}
	...
	if (color_mode < 3)
	{
		OnlyWayUi::Log::Message(OnlyWayUi::Log::LT_ERROR, "Only 24 and 32bit textures are supported.");
		return false;
	}

	const byte* image_src = buffer.get() + sizeof(TGAHeader);
	OnlyWayUi::UniquePtr<byte[]> image_dest_buffer(new byte[image_size]);
...
OnlyWayUi::TextureHandle RenderInterface_GL3::GenerateTexture(OnlyWayUi::Span<const OnlyWayUi::byte> source_data, OnlyWayUi::Vector2i source_dimensions)
{
	OWUI_ASSERT(source_data.data() && source_data.size() == size_t(source_dimensions.x * source_dimensions.y * 4));

	GLuint texture_id = Gfx::CreateTexture(source_data, source_dimensions);
	if (texture_id == 0)
	{
		OnlyWayUi::Log::Message(OnlyWayUi::Log::LT_ERROR, "Failed to generate texture.");
		return {};
	}
	return (OnlyWayUi::TextureHandle)texture_id;
}
...
void RenderInterface_GL3::DrawFullscreenQuad(OnlyWayUi::Vector2f uv_offset, OnlyWayUi::Vector2f uv_scaling)
{
	OnlyWayUi::Mesh mesh;
	OnlyWayUi::MeshUtilities::GenerateQuad(mesh, OnlyWayUi::Vector2f(-1), OnlyWayUi::Vector2f(2), {});
	if (uv_offset != OnlyWayUi::Vector2f() || uv_scaling != OnlyWayUi::Vector2f(1.f))
	{
		for (OnlyWayUi::Vertex& vertex : mesh.vertices)
			vertex.tex_coord = (vertex.tex_coord * uv_scaling) + uv_offset;
	}
	const OnlyWayUi::CompiledGeometryHandle geometry = CompileGeometry(mesh.vertices, mesh.indices);
	RenderGeometry(geometry, {}, RenderInterface_GL3::TexturePostprocess);
	ReleaseGeometry(geometry);
}

static OnlyWayUi::Colourf ConvertToColorf(OnlyWayUi::ColourbPremultiplied c0)
{
	OnlyWayUi::Colourf result;
	for (int i = 0; i < 4; i++)
...
static void SigmaToParameters(const float desired_sigma, int& out_pass_level, float& out_sigma)
{
	...
	out_pass_level = OnlyWayUi::Math::Clamp(OnlyWayUi::Math::Log2(int(desired_sigma * (2.f / max_single_pass_sigma))), 0, max_num_passes);
	out_sigma = OnlyWayUi::Math::Clamp(desired_sigma / float(1 << out_pass_level), 0.0f, max_single_pass_sigma);
}

static void SetTexCoordLimits(GLint tex_coord_min_location, GLint tex_coord_max_location, OnlyWayUi::Rectanglei rectangle_flipped,
	OnlyWayUi::Vector2i framebuffer_size)
{
	...
	const OnlyWayUi::Vector2f min = (OnlyWayUi::Vector2f(rectangle_flipped.p0) + OnlyWayUi::Vector2f(0.5f)) / OnlyWayUi::Vector2f(framebuffer_size);
	const OnlyWayUi::Vector2f max = (OnlyWayUi::Vector2f(rectangle_flipped.p1) - OnlyWayUi::Vector2f(0.5f)) / OnlyWayUi::Vector2f(framebuffer_size);
...
static void SetBlurWeights(GLint weights_location, float sigma)
{
	...
	{
		if (OnlyWayUi::Math::Absolute(sigma) < 0.1f)
			weights[i] = float(i == 0);
		else
			weights[i] = OnlyWayUi::Math::Exp(-float(i * i) / (2.0f * sigma * sigma)) / (OnlyWayUi::Math::SquareRoot(2.f * OnlyWayUi::Math::OWUI_PI) * sigma);
...
void RenderInterface_GL3::RenderBlur(float sigma, const Gfx::FramebufferData& source_destination, const Gfx::FramebufferData& temp,
	const OnlyWayUi::Rectanglei window_flipped)
{
	OWUI_ASSERT(&source_destination != &temp && source_destination.width == temp.width && source_destination.height == temp.height);
	OWUI_ASSERT(window_flipped.Valid());

	int pass_level = 0;
	SigmaToParameters(sigma, pass_level, sigma);

	const OnlyWayUi::Rectanglei original_scissor = scissor_state;

	// Begin by downscaling so that the blur pass can be done at a reduced resolution for large sigma.
	OnlyWayUi::Rectanglei scissor = window_flipped;
	...
	// neighboring texels.
	const OnlyWayUi::Vector2f uv_scaling = {(source_destination.width % 2 == 1) ? (1.f - 1.f / float(source_destination.width)) : 1.f,
		(source_destination.height % 2 == 1) ? (1.f - 1.f / float(source_destination.height)) : 1.f};

	for (int i = 0; i < pass_level; i++)
	{
		scissor.p0 = (scissor.p0 + OnlyWayUi::Vector2i(1)) / 2;
		scissor.p1 = OnlyWayUi::Math::Max(scissor.p1 / 2, scissor.p0);
	...
	auto SetTexelOffset = [texel_offset_location](OnlyWayUi::Vector2f blur_direction, int texture_dimension) {
		const OnlyWayUi::Vector2f texel_offset = blur_direction * (1.0f / float(texture_dimension));
	...
	const OnlyWayUi::Vector2i src_min = scissor.p0;
	const OnlyWayUi::Vector2i src_max = scissor.p1;
	const OnlyWayUi::Vector2i dst_min = window_flipped.p0;
	const OnlyWayUi::Vector2i dst_max = window_flipped.p1;
	...
	const OnlyWayUi::Vector2i target_min = src_min * (1 << pass_level);
	const OnlyWayUi::Vector2i target_max = src_max * (1 << pass_level);
...
void RenderInterface_GL3::ReleaseTexture(OnlyWayUi::TextureHandle texture_handle)
{
	glDeleteTextures(1, (GLuint*)&texture_handle);
}

void RenderInterface_GL3::SetTransform(const OnlyWayUi::Matrix4f* new_transform)
{
...
struct CompiledFilter {
	...
	// Drop shadow
	OnlyWayUi::Vector2f offset;
	OnlyWayUi::ColourbPremultiplied color;

	// ColorMatrix
	OnlyWayUi::Matrix4f color_matrix;
};

OnlyWayUi::CompiledFilterHandle RenderInterface_GL3::CompileFilter(const OnlyWayUi::String& name, const OnlyWayUi::Dictionary& parameters)
{
	...
	if (name == "opacity")
	{
		...
		filter.blend_factor = OnlyWayUi::Get(parameters, "value", 1.0f);
	}
	else if (name == "blur")
	{
		filter.type = FilterType::Blur;
		filter.sigma = OnlyWayUi::Get(parameters, "sigma", 1.0f);
	}
	else if (name == "drop-shadow")
	{
		filter.type = FilterType::DropShadow;
		filter.sigma = OnlyWayUi::Get(parameters, "sigma", 0.f);
		filter.color = OnlyWayUi::Get(parameters, "color", OnlyWayUi::Colourb()).ToPremultiplied();
		filter.offset = OnlyWayUi::Get(parameters, "offset", OnlyWayUi::Vector2f(0.f));
	}
	else if (name == "brightness")
	{
		filter.type = FilterType::ColorMatrix;
		const float value = OnlyWayUi::Get(parameters, "value", 1.0f);
		filter.color_matrix = OnlyWayUi::Matrix4f::Diag(value, value, value, 1.f);
	}
	else if (name == "contrast")
	{
		...
		filter.color_matrix = OnlyWayUi::Matrix4f::Diag(value, value, value, 1.f);
		filter.color_matrix.SetColumn(3, OnlyWayUi::Vector4f(grayness, grayness, grayness, 1.f));
	}
	else if (name == "invert")
	{
		filter.type = FilterType::ColorMatrix;
		const float value = OnlyWayUi::Math::Clamp(OnlyWayUi::Get(parameters, "value", 1.0f), 0.f, 1.f);
		const float inverted = 1.f - 2.f * value;
		filter.color_matrix = OnlyWayUi::Matrix4f::Diag(inverted, inverted, inverted, 1.f);
		filter.color_matrix.SetColumn(3, OnlyWayUi::Vector4f(value, value, value, 1.f));
	}
	else if (name == "grayscale")
	{
		...
		const OnlyWayUi::Vector3f gray = value * OnlyWayUi::Vector3f(0.2126f, 0.7152f, 0.0722f);
		// clang-format off
		filter.color_matrix = OnlyWayUi::Matrix4f::FromRows(
	...
	else if (name == "sepia")
	{
		...
		const float value = OnlyWayUi::Get(parameters, "value", 1.0f);
		const float rev_value = 1.f - value;
		const OnlyWayUi::Vector3f r_mix = value * OnlyWayUi::Vector3f(0.393f, 0.769f, 0.189f);
		const OnlyWayUi::Vector3f g_mix = value * OnlyWayUi::Vector3f(0.349f, 0.686f, 0.168f);
		const OnlyWayUi::Vector3f b_mix = value * OnlyWayUi::Vector3f(0.272f, 0.534f, 0.131f);
		// clang-format off
		filter.color_matrix = OnlyWayUi::Matrix4f::FromRows(
	...
	else if (name == "hue-rotate")
	{
		...
		filter.type = FilterType::ColorMatrix;
		const float value = OnlyWayUi::Get(parameters, "value", 1.0f);
		const float s = OnlyWayUi::Math::Sin(value);
		const float c = OnlyWayUi::Math::Cos(value);
		// clang-format off
		filter.color_matrix = OnlyWayUi::Matrix4f::FromRows(
	...
	if (filter.type != FilterType::Invalid)
		return reinterpret_cast<OnlyWayUi::CompiledFilterHandle>(new CompiledFilter(std::move(filter)));

	OnlyWayUi::Log::Message(OnlyWayUi::Log::LT_WARNING, "Unsupported filter type '%s'.", name.c_str());
	return {};
}

void RenderInterface_GL3::ReleaseFilter(OnlyWayUi::CompiledFilterHandle filter)
{
...
struct CompiledShader {
	...
	OnlyWayUi::Vector2f p;
	OnlyWayUi::Vector2f v;
	OnlyWayUi::Vector<float> stop_positions;
	OnlyWayUi::Vector<OnlyWayUi::Colourf> stop_colors;

	// Shader
	OnlyWayUi::Vector2f dimensions;
};

OnlyWayUi::CompiledShaderHandle RenderInterface_GL3::CompileShader(const OnlyWayUi::String& name, const OnlyWayUi::Dictionary& parameters)
{
	auto ApplyColorStopList = [](CompiledShader& shader, const OnlyWayUi::Dictionary& shader_parameters) {
		auto it = shader_parameters.find("color_stop_list");
		OWUI_ASSERT(it != shader_parameters.end() && it->second.GetType() == OnlyWayUi::Variant::COLORSTOPLIST);
		const OnlyWayUi::ColorStopList& color_stop_list = it->second.GetReference<OnlyWayUi::ColorStopList>();
		const int num_stops = OnlyWayUi::Math::Min((int)color_stop_list.size(), MAX_NUM_STOPS);

		shader.stop_positions.resize(num_stops);
		shader.stop_colors.resize(num_stops);
		for (int i = 0; i < num_stops; i++)
		{
			const OnlyWayUi::ColorStop& stop = color_stop_list[i];
			OWUI_ASSERT(stop.position.unit == OnlyWayUi::Unit::NUMBER);
	...
	if (name == "linear-gradient")
	{
		...
		const bool repeating = OnlyWayUi::Get(parameters, "repeating", false);
		shader.gradient_function = (repeating ? ShaderGradientFunction::RepeatingLinear : ShaderGradientFunction::Linear);
		shader.p = OnlyWayUi::Get(parameters, "p0", OnlyWayUi::Vector2f(0.f));
		shader.v = OnlyWayUi::Get(parameters, "p1", OnlyWayUi::Vector2f(0.f)) - shader.p;
	...
	else if (name == "radial-gradient")
	{
		...
		shader.p = OnlyWayUi::Get(parameters, "center", OnlyWayUi::Vector2f(0.f));
		shader.v = OnlyWayUi::Vector2f(1.f) / OnlyWayUi::Get(parameters, "radius", OnlyWayUi::Vector2f(1.f));
		ApplyColorStopList(shader, parameters);
	}
	else if (name == "conic-gradient")
	{
		...
		shader.p = OnlyWayUi::Get(parameters, "center", OnlyWayUi::Vector2f(0.f));
		const float angle = OnlyWayUi::Get(parameters, "angle", 0.f);
		shader.v = {OnlyWayUi::Math::Cos(angle), OnlyWayUi::Math::Sin(angle)};
		ApplyColorStopList(shader, parameters);
	}
	else if (name == "shader")
	{
		const OnlyWayUi::String value = OnlyWayUi::Get(parameters, "value", OnlyWayUi::String());
		if (value == "creation")
		{
			shader.type = CompiledShaderType::Creation;
			shader.dimensions = OnlyWayUi::Get(parameters, "dimensions", OnlyWayUi::Vector2f(0.f));
		}
	}

	if (shader.type != CompiledShaderType::Invalid)
		return reinterpret_cast<OnlyWayUi::CompiledShaderHandle>(new CompiledShader(std::move(shader)));

	OnlyWayUi::Log::Message(OnlyWayUi::Log::LT_WARNING, "Unsupported shader type '%s'.", name.c_str());
	return {};
}

void RenderInterface_GL3::RenderShader(OnlyWayUi::CompiledShaderHandle shader_handle, OnlyWayUi::CompiledGeometryHandle geometry_handle,
	OnlyWayUi::Vector2f translation, OnlyWayUi::TextureHandle /*texture*/)
{
	OWUI_ASSERT(shader_handle && geometry_handle);
	...
	{
		OWUI_ASSERT(shader.stop_positions.size() == shader.stop_colors.size());
	...
	{
		const double time = OnlyWayUi::GetSystemInterface()->GetElapsedTime();
	...
	{
		OnlyWayUi::Log::Message(OnlyWayUi::Log::LT_WARNING, "Unhandled render shader %d.", (int)type);
	}
	break;
	}
...
void RenderInterface_GL3::ReleaseShader(OnlyWayUi::CompiledShaderHandle shader_handle)
{
	delete reinterpret_cast<CompiledShader*>(shader_handle);
}

void RenderInterface_GL3::BlitLayerToPostprocessPrimary(OnlyWayUi::LayerHandle layer_handle)
{
...
void RenderInterface_GL3::RenderFilters(OnlyWayUi::Span<const OnlyWayUi::CompiledFilterHandle> filter_handles)
{
	for (const OnlyWayUi::CompiledFilterHandle filter_handle : filter_handles)
	{
		...
		case FilterType::Blur:
		{
			...
			const OnlyWayUi::Rectanglei window_flipped = VerticallyFlipped(scissor_state, viewport_height);
		...
		case FilterType::DropShadow:
		{
			...
			OnlyWayUi::Colourf color = ConvertToColorf(filter.color);
			...
			const OnlyWayUi::Vector2f uv_offset = filter.offset / OnlyWayUi::Vector2f(-(float)viewport_width, (float)viewport_height);
		...
		case FilterType::ColorMatrix:
		{
			...
			constexpr bool transpose = std::is_same_v<decltype(filter.color_matrix), OnlyWayUi::RowMajorMatrix4f>;
		...
		{
			OnlyWayUi::Log::Message(OnlyWayUi::Log::LT_WARNING, "Unhandled render filter %d.", (int)type);
		}
		break;
		}
	}
...
OnlyWayUi::LayerHandle RenderInterface_GL3::PushLayer()
{
	const OnlyWayUi::LayerHandle layer_handle = render_layers.PushLayer();
...
void RenderInterface_GL3::CompositeLayers(OnlyWayUi::LayerHandle source_handle, OnlyWayUi::LayerHandle destination_handle, OnlyWayUi::BlendMode blend_mode,
	OnlyWayUi::Span<const OnlyWayUi::CompiledFilterHandle> filters)
{
	using OnlyWayUi::BlendMode;
...
OnlyWayUi::TextureHandle RenderInterface_GL3::SaveLayerAsTexture()
{
	OWUI_ASSERT(scissor_state.Valid());
	// Snapshot before disabling scissor. Frame damage may have shrunk scissor_state relative to the
	// RenderManager region; the texture size follows the effective GL scissor.
	const OnlyWayUi::Rectanglei bounds = scissor_state;

	GLuint render_texture = Gfx::CreateTexture({}, bounds.Size());
	if (render_texture == 0)
	{
		OnlyWayUi::Log::Message(OnlyWayUi::Log::LT_ERROR, "Failed to create render texture.");
		return {};
	}

	// Resolve the full layer without scissor. A scissored MSAA blit can leave a corrupt strip on the
	// scissor edge (visible as a frozen top band when that edge is the capture top).
	...
	return (OnlyWayUi::TextureHandle)render_texture;
}

OnlyWayUi::CompiledFilterHandle RenderInterface_GL3::SaveLayerAsMaskImage()
{
	BlitLayerToPostprocessPrimary(render_layers.GetTopLayerHandle());
...
void RenderInterface_GL3::SubmitTransformUniform(OnlyWayUi::Vector2f translation)
{
...
OnlyWayUi::LayerHandle RenderInterface_GL3::RenderLayerStack::PushLayer()
{
	OWUI_ASSERT(layers_size <= (int)fb_layers.size());
	...
	if (layers_size == (int)fb_layers.size())
	{
		...
		Gfx::CreateFramebuffer(fb_layers.back(), width, height, OWUI_NUM_MSAA_SAMPLES, Gfx::FramebufferAttachment::DepthStencil,
...
{
	OWUI_ASSERT(layers_size > 0);
	layers_size -= 1;
}

const Gfx::FramebufferData& RenderInterface_GL3::RenderLayerStack::GetLayer(OnlyWayUi::LayerHandle layer) const
{
	OWUI_ASSERT((size_t)layer < (size_t)layers_size);
...
OnlyWayUi::LayerHandle RenderInterface_GL3::RenderLayerStack::GetTopLayerHandle() const
{
	OWUI_ASSERT(layers_size > 0);
	return static_cast<OnlyWayUi::LayerHandle>(layers_size - 1);
}
...
{
	OWUI_ASSERT(layers_size == 1);
	PopLayer();
}

void RenderInterface_GL3::RenderLayerStack::DestroyFramebuffers()
{
	OWUI_ASSERTMSG(layers_size == 0, "Do not call this during frame rendering, that is, between BeginFrame() and EndFrame().");
...
{
	OWUI_ASSERT(index < (int)fb_postprocess.size())
...
const OnlyWayUi::Matrix4f& RenderInterface_GL3::GetTransform() const
{
	return transform;
}
...
bool OwuiGL3::Initialize(OnlyWayUi::String* out_message)
{
	const int gl_version = OwuiGL3Loader::Load();
	if (gl_version == 0)
	{
		if (out_message)
			*out_message = "Failed to initialize OpenGL 3.3 context.";
		return false;
	}

	if (out_message)
		*out_message = OnlyWayUi::CreateString("Loaded OpenGL %d.%d.", OWUI_GL3_VERSION_MAJOR(gl_version), OWUI_GL3_VERSION_MINOR(gl_version));

	return true;
}

void OwuiGL3::Shutdown()
{
	OwuiGL3Loader::Unload();
}
