...
For failure details, run a test binary directly: `./build/scintilla/test/unit/unitTest` for the upstream platform-free tests or `./build/scintilla/test/editor/editorTest` for the concrete editor tests (Catch2 v2; pass a test name pattern to run one case). The core builds as a static library, `scintilla_core`. The unit executable links only the platform-free objects it calls. `editorTest` links the editor concern translation units (`Editor*.cxx`, `ScintillaBase.cxx`, and their required core objects) against the deterministic test-only `Platform.h` implementation, so missing editor definitions fail the build. Named feature work lives in concern files such as `EditorWrapping.cxx` and `EditorDocument.cxx`; `Editor.cxx` keeps shared paint, geometry, notification, and the temporary dispatch shell. The first application with a real platform implementation arrives in roadmap phase 6.
