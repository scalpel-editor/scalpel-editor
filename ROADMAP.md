...
3. Implement the shaped-run model and cache. ✅ (2026-07-16) Shape directional runs with HarfBuzz, disable discretionary ligatures, split fallback spans without losing input-byte offsets, and cache glyph IDs, advances, offsets, byte clusters, caret stops, font faces, and direction. Tests cover ASCII, kerning on the existing primary fixture, multi-byte clusters, invalid UTF-8, required left-to-right shaping, and deterministic font fallback; combining-mark sequences and mixed-direction line ordering remain out of scope.
...
6. Add glyph rasterization and text drawing. Rasterize the shaped glyphs with FreeType, cache them in the renderer, and draw using the shaped advances and offsets rather than measuring again. Image checks with the fixture fonts cover text, clipping, caret, selection, indicators, margins, scrolling, combining sequences, required shaping, and fallback, while numeric assertions compare rendered glyph and selection bounds with layout results.
