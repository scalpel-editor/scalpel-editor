...
10. Delete the remaining generated client and documentation surface. ✅ (2026-07-16) Deleted `ScintillaCall.h`, `Scintilla.h` (no constants needed re-homing), `Scintilla.iface`, GTK-only `ScintillaWidget.h`, and `ScintillaDoc.html` after confirming retained prose lives beside named implementations. KeyMap default bindings use `KeyMod` instead of local `SCI_*` aliases. Discoverability obsolete paths, AGENTS, DISCOVERABILITY, MESSAGE_REMOVAL, and the phase 5 boundary no longer treat the deleted files as live input.
11. Run completion searches and the phase gate. Update grepai for every replacement header and affected source, run exact-name and cold-navigation checks, and record the results. Search the whole repository for the forbidden forms below, run the normal workflow, run the editor link test explicitly, and run `./check.sh` before marking the phase complete. ✅ (2026-07-16)

Done for step 11: all 91 Phase 5-changed C++ source and header files, plus the final audit corrections, were confirmed in grepai's durable index before a clean-revision 240-search matrix. Under vector search over the whole repository, 28 of 33 retained natural-language queries (84.8%) place the expected concern in the top three; exact-name `rg` checks pass and cold navigation reaches every sampled implementation within two searches. `tools/check-no-message-layer.sh` finds no dispatch, generated client file, message constant, coercion helper, client packing type, or live documentation directing readers through the deleted path; its narrow external-contract allowances are recorded with owners and reasons. The normal workflow, explicit `Test editor contract` construction/link case, and complete `dev` / `asan` / `ubsan` matrix pass. Final evidence is in `benchmark-results/phase5-final/`.
