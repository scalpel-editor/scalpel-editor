...
6. Add glyph rasterization and text drawing. Rasterize the shaped glyphs with FreeType, cache them in the renderer, and draw using the shaped advances and offsets rather than measuring again. Image checks with the fixture fonts cover English text, clipping, caret, selection, indicators, margins, scrolling, combining sequences, kerning, and fallback, while numeric assertions compare rendered glyph and selection bounds with layout results.
7. Collapse the platform layer onto the concrete implementation. Remove renderer-selection arguments, opaque surface handles, virtual allocation paths, and other cross-platform choices that no longer serve this program; update the core and printing path to use the one surface and font implementation directly. Replace the editor fixture's recorded drawing surface with the renderer's deterministic offscreen target while retaining its observable host callbacks, and give popup operations explicit recorded-failure stubs.
8. Add the application editor host and executable. Create the production `ScintillaBase` subclass with direct invalidation, size, focus, cursor, capture, scrollbar, ticker, idle, notification, and drawing paths; load a fixed initial buffer and render it offscreen first. Keep clipboard, IME, popup, and other deferred requests visible as unsupported instead of reporting success, and retain the construction/link test for the complete editor path.
9. Bring up the minimal Wayland and EGL window. Generate only the required xdg-shell protocol code; connect to the display, bind the compositor and current seat, complete the initial xdg-surface configure handshake, create the xdg-toplevel and `wl_egl_window`, choose an EGL configuration, and create the context and window surface with explicit reverse-order cleanup. A smoke test reaches a configured window and displays the editor buffer; robust global and seat removal belongs to phase 7.
10. Implement the event loop and window lifecycle. Dispatch Wayland events together with editor tickers and idle work, redraw invalidated regions, swap completed frames, apply configure sizes to the EGL window and editor, propagate keyboard focus, and handle close and display failure without leaks or busy waits. Deterministic tests isolate timer scheduling, resize coalescing, focus transitions, redraw requests, and shutdown state from the live compositor.
11. Add basic input for the vertical slice. Build the current xkbcommon keymap from the seat, translate key press and release into text insertion or `EditorCommand`, and route pointer motion, buttons, wheel axes, capture, and editor coordinates. Tests cover modifiers, bound commands, unbound UTF-8 text, clicks, drags, and wheel scrolling; compose, repeat, IME, cursor themes, hot-plugged seats, and scale conversion remain phase 7 work.
12. Remove absorbed seed code and run the phase gate. Delete `seed/editor/PlatOWUI.*` and `seed/backends/OnlyWayUi_Renderer_GL3.*` after their useful code is represented directly, retain the backend references owned by phase 7, preserve required OnlyWayUi notices on derived code, update build and architecture documentation, run self-contained-header checks for changed headers, run the normal workflow and representative image tests explicitly, perform a manual Wayland smoke test, and run `./check.sh`.
