...
Complete the standalone shell by replacing the useful Wayland backend reference code with direct production ownership and implementing missing behavior from the relevant protocol and library sources. Optional protocols must have explicit, tested fallback behavior. Keep the shell readable as a worked C++ Wayland application rather than turning it into a general framework. Only the unabsorbed Phase 8 portal URI reference remains under `seed/`.

1. Freeze the Phase 7 boundary and test strategy. ✅ (2026-07-22) [tools/phase7-boundary.md](tools/phase7-boundary.md) inventories required and optional globals, generated protocols and fallbacks, library dependencies, retained seed sources and licenses, application host hooks, coordinate spaces, and deterministic seams and focused targets for callbacks, timing, transfers, IME, frames, scaling, and event-loop sources.
2. Make registry, output, and seat lifecycles robust. ✅ (2026-07-22) Registry names and output membership now live in deterministic lifecycle state; the shell tracks hot-plugged outputs and seats, keeps the earliest advertised seat active until removal, promotes a fresh replacement, clears stale input and focus before releasing keyboard and pointer objects, and closes cleanly when a required global disappears.
3. Complete toplevel and pointer behavior. ✅ (2026-07-22) The shell now negotiates optional server-side decoration before the initial surface commit, retains configured bounds, window state, capabilities, and decoration mode, tracks decoration-manager replacement, and coalesces current pointer-axis frames into one scroll event while preserving legacy compositor delivery without double-counting compatibility values.
4. Complete keyboard input. ✅ (2026-07-22) Keyboard text now passes through locale-aware xkbcommon compose with direct-input fallback when no compose table is available; compositor rate and delay drive a controlled-clock repeat scheduler whose deadlines join the Wayland wait and whose activity is cancelled on release, focus or capability loss, keymap replacement, and seat removal.
5. Add themed cursors. ✅ (2026-07-22) The shell now resolves the configured cursor theme and size, maps every editor cursor through ordered standard and compatibility names to an arrow fallback, applies scaled cursor buffers with the current pointer-entry serial, retains requests across unavailable services and pointer changes, and rebuilds theme resources when shared-memory or scale state changes.
6. Add clipboard transfers. ✅ (2026-07-22) The shell now owns hot-pluggable data-device managers, seat-bound clipboard devices, UTF-8 offers and sources, current input serials, and bounded nonblocking reads and writes whose descriptors and deadlines join the Wayland wait; the editor exchanges value requests and results with the shell, reports failures, and applies successful paste text only to the current request and document.
7. Add primary selection. ✅ (2026-07-22) The shell now owns an optional primary-selection manager, seat device, offers, sources, serials, and bounded transfers independently from the clipboard; editor selections publish after completed pointer drags, middle-button requests paste at the retained click position, and manager absence, replacement, stale offers, cancellation, seat loss, and superseded documents have deterministic fallback coverage.
8. Add text-input-v3 IME support. ✅ (2026-07-22) The shell now owns a hot-pluggable text-input manager and seat object, enables input only with surface and keyboard focus plus editor state, applies serial-checked pre-edit, commit, and byte deletion batches in protocol order, synchronizes bounded UTF-8 surrounding text and logical cursor rectangles, cancels tentative edits on lifecycle loss, and retains direct keyboard input when the optional protocol is absent.
9. Pace and report frames. ✅ (2026-07-22) One-shot `wl_surface.frame` callbacks now gate submissions without idle redraw spin; captured invalidation survives callback waits and painting, bounded buffer-age history extends repaint damage, EGL uses KHR or EXT damage swaps with full fallbacks, and optional presentation feedback reports each submitted frame by identifier without controlling pacing.
10. Make scaling coherent. ✅ (2026-07-23) One scale state now selects integer or fractional configuration from surface and output changes, optional viewporter and fractional-scale managers are hot-pluggable with integer fallback, and each update keeps logical editor input and IME geometry aligned with scaled rendering, EGL buffers, frame damage, and cursor resources.
11. Prepare the event loop for portal dialogs. ✅ (2026-07-23) One poll plan now merges Wayland, selection-transfer, and enabled D-Bus descriptors with editor, repeat, transfer, and D-Bus deadlines; a lazy private session-bus connection safely handles changing watches, timeouts, and pending dispatch; and optional xdg-foreign export retains the current `wayland:` parent token through exporter removal and replacement without adding file-dialog policy.
12. Integrate the completed shell and run the phase gate. ✅ (2026-07-23) Added six deterministic integration cases for the frozen cross-concern lifecycle sequences; deleted the absorbed backend, editor-host, sample, and seed-build references while retaining the Phase 8 portal URI converter; and updated the architecture record. A later source audit confirmed that the retained converter and the deleted Wayland backend were written after OnlyWayUi diverged from RmlUi, while the RmlUi-derived GL3 renderer had been fully replaced, so the temporarily retained copy of the RmlUi notice was obsolete and removed. All 75 production headers compiled alone, the normal workflow and all six application CTest entries passed, and the current KWin session advertised every Phase 7 optional global while the editor remained live for five seconds and reported 11 presented frames. Interactive input, transfer, IME, scale-change, and compositor-removal paths were not claimed as live because this runner could not drive them; their deterministic cases passed. The complete `dev` / `asan` / `ubsan` matrix passed all 11 tests in each tree.

Deliverable: keyboard compose and repeat, IME pre-edit and commit, pointer input, cursors, clipboard, primary selection, focus and seat changes, scaling, resize, frame callbacks, and presentation feedback work in the standalone editor; protocol-unavailable paths are tested; all absorbed backend seed files are deleted.
