...
12. Integrate the completed shell and run the phase gate. Exercise protocol-unavailable paths and combined lifecycle transitions, delete all absorbed backend seed files while retaining required license notices, update architecture documentation, run the normal workflow and live Wayland checks, then pass the complete `dev` / `asan` / `ubsan` matrix.
