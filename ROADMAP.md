...
In progress: asynchronous portal open and save dialogs, host load/dirty/mark-saved methods, atomic whole-file saves (temp file plus rename, symlink follow and mode preserve), and Ctrl+O / Ctrl+S / Ctrl+Shift+S bindings are in place. Dirty-buffer close uses an in-window centered Save / Discard / Cancel card drawn after Scintilla paint; portals stay for path picking only. Force-close from required-global loss still exits without a prompt. Portal response code 1 is cancel and other nonzero codes are failure; the predicted Request path is tracked before the method call.

Tabs are in place (2026-07-24). One `ApplicationEditor` retains multiple Scintilla documents; `DocumentWorkspace` owns ordered tabs, paths, untitled numbering, portal request intents, and dirty-close walks; `main` is the Wayland event and rendering adapter. Ctrl+N creates tabs, Ctrl+W closes them, and Ctrl+Tab cycles them; Ctrl+O opens each accepted path in its own tab (an existing lexically normalized path selects its tab); delayed Save As targets the initiating tab; dirty tab and multi-dirty window close prompt in strip order; closing the last tab creates a fresh untitled. A permanent tab strip reserves top chrome with damage-aware paint. [tools/phase8-tabs.md](tools/phase8-tabs.md) records the ownership boundary and nine-commit sequence.

The in-window menu bar foundation is in place (2026-07-25). Permanent top chrome is the menu bar above the tab strip, and open dropdowns use the post-paint overlay slot. `ApplicationAction` is the compile-time File / Edit command list shared by keyboard shortcuts and menu activation; `MenuBar` owns Wayland-free layout, hit testing, navigation, and paint; `main` owns the model, input priority, and shell requests. F10/Menu and Alt+F / Alt+E open the fixed-action menus; pointer and keyboard run the same dispatcher as Ctrl+N / O / S / Q and the edit shortcuts. [tools/phase8-menu-bar.md](tools/phase8-menu-bar.md) records the product boundary and eight-commit sequence. Remaining Phase 8 work also includes external-file change detection and conflict handling (now attachable to per-document workspace state), scrollbar and find bar chrome, invalid-UTF-8 and line-ending file policy, oversized-file handling, and the other daily-usable file-lifecycle checks above.
...
Scrollbars are in place (2026-07-25). Fixed in-window vertical and horizontal bars reserve client space around Scintilla; `ApplicationEditor` publishes exact two-axis metrics and clamped scroll methods; `ScrollBar` owns Wayland-free layout, hit testing, drag, and paint; permanent chrome now includes top, side, bottom, and junction regions with narrow invalidation. Pointer priority is file error, unsaved card, open menu, active scrollbar drag, top chrome, scrollbar hit, then editor; editor selection capture still wins over bar hits. [tools/phase8-scrollbars.md](tools/phase8-scrollbars.md) records the product boundary and seven-commit sequence. Remaining Phase 8 work also includes external-file change detection and conflict handling, find bar chrome, invalid-UTF-8 and line-ending file policy, oversized-file handling, and the other daily-usable file-lifecycle checks above.
