...
5. Build one renderer with an offscreen target. Absorb the useful GL3 seed techniques into direct code for transforms, clipping, solid and alpha geometry, gradients, images, copying, and target readback; the window and offscreen paths must execute the same drawing implementation. Pixel tests cover primitive bounds, nested clips, alpha blending, and image placement without a Wayland compositor.
