...
1. Freeze the removal boundary. While `Scintilla.iface` and the phase 4 verifiers still exist, record which generated enums, constants, structures, and notifications are used by production core code, the Lexilla-facing interfaces, and tests. Classify every generated definition as retained project data or client/message-only material, add a repeatable completion check for the forbidden message-layer forms listed below, and preserve the phase 4 inventory as historical evidence rather than making later work reconstruct it after the interface file is gone.
2. Remove message-shaped parameters from named code. Replace remaining `uptr_t` / `sptr_t`, pointer packing, sentinel lengths, and combined key-and-modifier values in production operations with the existing enum types, `Sci::Position`, `Sci::Line`, `size_t`, pointers, `std::string_view`, `std::optional`, or small typed records as appropriate. This includes policy setters, selection mode, search, text width, optional colours, and key binding. Stop every non-shell use of conversions such as `CommandFromMessage`, leaving their deletion with the forwarders in step 6, and add direct tests for any path whose typed form changes.
3. Convert the `Editor` concern tests off messages. Replace application-facing calls and parity checks with public named methods or `RunCommand`; expose protected concern operations only through narrow typed methods on the test-only `TestEditor`. Migrate concern by concern so each focused suite stays green, and do not promote private operations into the production public API for test convenience.
4. Convert the `ScintillaBase` and host tests off messages. Exercise autocomplete, call tips, lexer operations, popup command routing, and other subclass behavior through named typed entry points. Remove the fallback-message snapshot and `DefWndProc` observation from the concrete test editor after no test depends on them. Completion searches must then find no test or production call to `WndProc`.
5. Delete the `ScintillaBase::WndProc` forwarding shell. Remove its declaration, definition, message includes, parameter conversions, and the comments that describe temporary forwarding. Build and run the autocomplete, call-tip, lexing, popup-host, and editor-link tests before proceeding.
6. Delete the `Editor::WndProc` shell and compatibility helpers. Remove `WndProc`, `DefWndProc`, `StringResult`, `BytesResult`, pointer and integer coercion helpers, the remaining command-message map, and message-only includes. Keep the concrete editor construction/link test so the static archive cannot hide a missing named definition. At this point `ScintillaMessages.h` must have no consumer and can be deleted.
7. Replace the generated notification contract. Define project-owned notification kinds and data containing only fields emitted by the retained core, update the host callback and observation types, and remove the Windows-shaped `NotifyHeader`. Remove `Notification::MacroRecord` and the `Message`, `wParam`, and `lParam` recording fields; typed recording remains solely through `RecordedAction`. Keep focused tests for every retained notification kind used by the core.
8. Replace generated range and printing structures. Move retained range, find, and print data into hand-maintained project definitions close to their owners, using a shared header only when multiple concerns need the same type. Use `Sci::Position` and the concrete surface and rectangle types directly, and delete unused 32-bit and client-message variants. Delete `ScintillaStructures.h` when its last include is gone.
9. Replace generated enums and constants in concern-sized batches. Move the retained parts of `ScintillaTypes.h` into small hand-maintained definitions, starting with shared position and colour types, then document/history/search, styling/markers/decorations, input/selection/commands, and wrapping/folding/lexing. Preserve values used by retained behavior, masks and identifier ranges, and the Lexilla contract; delete only definitions classified as client/message-only in step 1. Run the matching focused suites after each batch, then delete `uptr_t` / `sptr_t` and `ScintillaTypes.h` when their last consumers are gone.
