...
| `ApplicationEditor` | Scintilla documents, per-document language, editor input, rendering, damage, editor work deadlines, editor client geometry, scrollbar visibility and ranges, clipboard values, text-input state, wrapped plain-text find, and the process-wide generic editor text face. |
...
9. Open emoji completion list (a row click completes; a press outside dismisses with click-through).
10. Visible find bar band (field, Previous, Next, Close, or empty chrome).
11. Permanent chrome: menu bar, tab strip, or a scrollbar hit.
12. Editor client (including a right press that opens the context menu when eligible).
...
Markdown token presentation is view state, not document state. Ordinary text stays on the selected body face. Strong text and complete ATX heading lines are bold; the bundled lexer styles only the underline marker of a Setext heading. Emphasis is italic. Code uses a monospace face and a restrained background. Links, quotes, list markers, rules, and strikeout tokens have distinct colours. Point size is unchanged. The same path runs at startup and after every Font menu choice so the palette cannot be erased.
...
Each retained document has a host-owned language, `PlainText` or `Markdown`, defaulting to `PlainText`. `SetDocumentLanguage` changes one document. An active document applies the change immediately; an inactive document stores the request and applies it on the next activation. Markdown attaches a fresh Lexilla Markdown lexer. The next paint styles the range the editor requests. Returning to plain text releases the lexer, clears lexer-produced style bytes and decorations, and leaves style 0 as the presentation. Language changes do not alter document bytes, undo history, save points, selections, or file identity.

A successful path bind whose stored path changes applies `DocumentLanguageFromPath`: ASCII case-insensitive `.md` and `.markdown` select Markdown; untitled documents and every other suffix stay plain text. Classification uses the normalized path, not the tab label or a portal MIME type. Interactive Open, Recent, startup load, and successful Save As classify after the bind succeeds. Save to an unchanged path, external-change reload, duplicate-path selection, and cancelled or failed operations leave the existing language. A delayed Save As can reclassify an inactive tab without activating it; the lexer change is applied on the next activation.
...
Lexically normalized paths identify open tabs. Open selects an existing binding instead of creating a duplicate. Save As refuses a destination already bound to another tab so two buffers cannot claim one path and overwrite each other on save. A successful open or Save As that changes the bound path applies that path's language to the tab.
...
The Edit menu also provides `Increase Quote Level` (`Ctrl+'`) and `Decrease Quote Level` (`Ctrl+Shift+'`). Increase inserts `> ` at the start of each selected line, including empty lines and nested quotes. Decrease removes one CommonMark block-quote marker (0-3 leading spaces, `>`, and one following space when present) from each selected line that has one. A caret or in-line selection affects that line; a multi-line selection uses the lines that contain selected characters and omits a trailing line when the selection ends at that line's start. Either command is one undo action, may run on any document language, and treats list items, headings, and fenced code as ordinary lines.
...
- Emoji completion is application-owned, not Scintilla autocomplete. Typing `:` at a word boundary (start of the line or after a byte that is not a shortcode character and not a UTF-8 lead or continuation byte) opens a caret-anchored list of GitHub shortcode matches. Further typing filters by alias prefix, so `+1` and `thumbsup` select the same thumbs-up emoji. Enter, Tab, or a row click replaces the live `:query` with the Unicode emoji as one undo action. Escape leaves the typed text. The list paints with permanent chrome rather than `BoundOverlay` or an `xdg_popup`, stays inside the editor client, and uses whichever side of the caret fits more rows when the full list does not fit. A modal, menu, context menu, find-field focus, application command, tab change, resize, or focus loss dismisses it. `https:` and `12:00` do not open it. The list is available in every document, not only Markdown.
