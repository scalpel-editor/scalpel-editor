...
`GlContext` owns EGL and OpenGL context state. A window context draws to framebuffer 0 through the `wl_egl_window`; a headless context owns an offscreen target. `Renderer` owns drawing programs, glyph textures, and the requested draw target, clip stack, and output scale. `DrawSurface` implements Scintilla's measurement and drawing surface contract over that renderer.

Applied framebuffer, viewport, and scissor live on `GlContext` so an editor renderer and a popup renderer that share the context observe one record. Framebuffer 0 is a real binding; unknown state is stored separately from that name. This is not a general GL state cache: shader, texture, blend, VAO, and uniform state are not tracked.

At an external drawing boundary (`DrawSurface::BindDrawTarget`, public renderer clip/clear/draw operations, frame-entry `MakeCurrent`, and popup renderer construction), the context uses thread-local tracking to skip unchanged context and surface selection. When tracking does not establish a match, it checks the actual EGL context and draw/read surfaces and calls `eglMakeCurrent` only when they differ. Context and surface changes must go through `GlContext` to keep this tracking valid. Destroyed contexts and missing popup surfaces are rejected before any fast return. Tracking is updated only after a successful bind or a matching EGL query. Release, destruction, surface replacement, and failed transitions forget that the surfaces are current. Switching the bound window surface also forgets the applied viewport and scissor, because framebuffer 0 then names a different window. Scissor enablement and box validity are tracked separately: disabling the test does not establish an unknown box.

Unchanged target selection preserves clips and applies the final scissor once. Buffer or logical size changes rebuild clips; a physical height change recomputes bottom-left scissor coordinates. Viewport updates when physical dimensions change or the applied viewport is unknown. Colour-buffer allocation and readback restore incoming draw and read framebuffer bindings, including when those two names differ. Deletion preserves bindings to other framebuffers; deleting a bound framebuffer lets GL reset that binding to 0. Resize deletes the old framebuffer before preserving bindings around allocation of its replacement. Deleting a framebuffer whose name is the applied binding marks that binding unknown so a later reuse of the name cannot skip a required bind.

A text run uses an internal prepared drawing scope after background fill and clip setup so glyph submission and cold uploads do not repeat boundary checks. Early returns and exceptions cannot leave that scope set. Pixmap and popup work can interrupt a frame, so preparation is not moved to frame start; standalone public renderer operations still establish state after intervening context or resource work.
...
Each drawing surface keeps bounded, weakly owned ink bounds for the most recent placement of a shaped run. A warm run outside the active clip skips all glyph visits without changing its shaping input. Placement, nominal scale, and logical/buffer dimensions must match; otherwise drawing refreshes the bounds through the exact glyph path. Background fills retain their original geometry.
...
The frame surface normalizes damage into disjoint rectangles, rounds them outward to buffer pixels, and normalizes again to remove fractional-scale overlaps. Both normalizations retain at most 16 rectangles, falling back to their bounding rectangle if fragmentation exceeds that limit. Each region has an exact outer buffer clip, restored when pixmap drawing changes targets, and a corresponding logical area for row selection. Text, margins, and permanent chrome cannot modify pixels outside that region. Offscreen and window frames use the same composition path, and EGL swap damage is built from the exact buffer clips with bottom-left coordinates. Wayland submission damage still describes current changes separately from older repaint regions required by buffer age.
...
Scintilla paints the editor client. A permanent-chrome callback paints the menu bar, tab strip, scrollbars, and scrollbar junction once per intersecting repaint region under its buffer clip, without forcing full-frame damage. A post-paint overlay callback paints exactly one open menu, external-change confirmation card, large-file confirmation card, unsaved-changes card, or file-error card; transparent overlays expand painting to the full frame and use a full swap so preserved pixels cannot accumulate blending. Overlay selection priority is file-error card, external-change confirmation, large-file confirmation, unsaved-changes prompt, open menu, then none.
...
Autocomplete lists and call tips use explicit production stubs. Their core behavior remains compiled and testable, but real Wayland popup surfaces for those features are not implemented. Production `ListBox::Create` leaves no `WindowID`, so `Window::Created` stays false; `AutoComplete::Cancel` still clears `Active` and the in-memory list. `popupStubTest` links those production stubs; `editorTest` uses an inspectable test list box instead. The application context menu paints into a separate grabbed `xdg_popup` EGL window surface that shares the editor GL context. Each renderer retains its selected EGL surface, and the popup renderer is destroyed before the context returns to the editor surface. Failed popup paint restores the editor surface in the same way, then destroys the popup. The next editor `BindDrawTarget` reapplies viewport and scissor after that surface switch. The popup buffer scale and viewport match the toplevel so popup-local input and painted logical coordinates agree.

## Testing

Offscreen renderer tests use the production OpenGL path with controlled fixture fonts. Outline text checks include reference composition against phase-aware FreeType coverage (not merely repeated-draw stability). Glyph-cache tests cover outline scale and phase identity with immediate retirement on output-scale change, and fixed-bitmap physical texture sizes with three-generation shrink reuse and eviction. Colour emoji edge checks require soft coverage after opaque composition rather than reading source alpha from the blended target. Synthetic CPU reducer tests own exact mass and fringe expectations. Host Fontconfig checks remain non-pixel-exact. Editor tests paint through the same `DrawSurface` implementation while retaining host observations. Application-host scale tests own nominal `RasterScale` identity and outline retirement through `ApplicationEditor`; fixed-bitmap sizing is exercised on the renderer side of that handoff. Partial repaint tests compare preserved pixels with full-render references, check warm run and glyph rejection counters, and cover sparse row painting, styling changes in gaps, fractional clip overlap, chrome, and recovery after painter failure. Renderer state tests cover repeated preparation, context release, alternating renderers on one context, colour-buffer allocation between draws, and two renderers targeting framebuffer 0 at equal and unequal sizes. Those headless cases prove shared applied GL state; they cannot create two EGL window surfaces. Wayland popup lifecycle tests cover protocol configure/ack/destroy, not real EGL surface switching. Production popup paint against a compositor is the path that switches the shared context from the editor window surface to the popup window surface and back. Wayland frame and scale tests verify damage conversion and nominal raster-scale ownership separately from live compositor submission.
