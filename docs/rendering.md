...
At an external drawing boundary (`DrawSurface::BindDrawTarget`, public renderer clip/clear/draw operations, frame-entry `MakeCurrent`, and popup renderer construction), the context checks the actual EGL context and draw/read surfaces and calls `eglMakeCurrent` only when they differ. Destroyed contexts and missing popup surfaces are rejected before any fast return. Tracking is updated only after a successful bind. Release, destruction, surface replacement, and failed transitions forget that the surfaces are current. Switching the bound window surface also forgets the applied viewport and scissor, because framebuffer 0 then names a different window.

Unchanged target selection preserves clips and applies the final scissor once. Buffer or logical size changes rebuild clips; a physical height change recomputes bottom-left scissor coordinates. Viewport updates only when physical dimensions change. Colour-buffer resize, readback, and deletion restore incoming draw and read framebuffer bindings, including when those two names differ. Deleting a framebuffer whose name is the applied binding marks that binding unknown so a later reuse of the name cannot skip a required bind.
