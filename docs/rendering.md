...
`ApplicationUi` owns the chrome models, hover and press state, and which overlay is selected. The shell still applies input priority and painter binding for now. Individual controls own their layout, hit testing, interaction state, and paint operations without a widget hierarchy.
