...
`main.cxx` is the application coordinator and platform adapter. It moves values among the window, editor, workspace, and fixed chrome; applies the input-priority order; submits frames; and waits for the next external event or application deadline. Wayland transport does not decide document-close or presentation policy.
