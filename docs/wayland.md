...
`main.cxx` is the platform adapter and event pump: construction, Wayland event collection, external-service transport, frame submission, and waiting. `ApplicationUi` is the Wayland-free application coordinator. It owns composition and input priority, consumes workspace requests and outcomes, and returns only typed portal-dialog and accepted-close effects to the host. `main.cxx` maps portal request IDs to application dialog identities, feeds copied results back through `ApplicationUi`, submits editor damage, and waits for the next external event or application deadline.
