...
The shell now has robust core-global, output, and seat lifetimes plus composed and repeating keyboard input, themed cursors, asynchronous clipboard and primary-selection transfers, and text-input-v3 IME batches. `WaylandWindow` owns the display, registry, required globals, xdg-toplevel, bound outputs, active seat devices, main and cursor surfaces, cursor theme, transfer and text-input managers, and `wl_egl_window`; thin callbacks feed deterministic registry, output-membership, seat-capability, window, cursor, input, offer, transfer, and IME state. `ApplicationEditor` owns Scintilla host state, drawing, invalidation, editor deadlines, idle work, transfer values, surrounding text, cursor rectangles, and tentative IME edits. `main.cxx` moves resize, input, transfer, and IME values between the shell and editor and returns the retained cursor request, while the Wayland wait combines editor, key-repeat, and transfer deadlines with display and transfer descriptors.
...
`waylandLifecycleTest` now covers resize, close, registry identity, required-global loss, output membership, active-seat replacement, device-capability actions, optional-manager replacement, retained toplevel state, and cursor choice and state; `waylandInputTest` covers translated and composed keyboard events, controlled repeat timing, pointer-axis frames, text-input batches, and stale-event clearing on device teardown; `waylandTransferTest` covers plain clipboard state and real nonblocking pipe transfers; `applicationTest` covers host behavior including tentative IME edits. The remaining frame pacing and scale conversion still live in or would otherwise be added directly to `WaylandWindow`, where they cannot be driven without a compositor. That framework deficiency is removed concern by concern in the later Phase 7 steps.
