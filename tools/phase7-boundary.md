...
The shell now has robust core-global, output, and seat lifetimes plus composed and repeating keyboard input, themed cursors, asynchronous clipboard and primary-selection transfers, text-input-v3 IME batches, compositor-paced damaged frames with optional presentation reports, coherent scaling, and a poll plan ready for lazy session D-Bus work. `WaylandWindow` owns the display, registry, required globals, xdg-toplevel, bound outputs, active seat devices, main and cursor surfaces, cursor theme, transfer and text-input managers, frame callback and presentation objects, optional D-Bus connection, and `wl_egl_window`; thin callbacks feed deterministic registry, output-membership, seat-capability, window, cursor, input, offer, transfer, IME, frame, presentation, scale, and event-loop state. `ApplicationEditor` owns Scintilla host state, drawing, invalidation, editor deadlines, idle work, transfer values, surrounding text, cursor rectangles, and tentative IME edits. `main.cxx` moves resize, input, transfer, IME, and frame damage values between the shell and editor and returns the retained cursor request, while one poll snapshot combines Wayland, transfer, and enabled D-Bus descriptors with editor, key-repeat, transfer, and D-Bus deadlines.
...
`waylandLifecycleTest` now covers resize, close, registry identity, required-global loss, output membership, active-seat replacement, device-capability actions, optional-manager replacement, retained toplevel state, cursor choice and state, frame pacing, damage conversion, buffer age, presentation results, and scale selection; `waylandInputTest` covers translated and composed keyboard events, controlled repeat timing, pointer-axis frames, text-input batches, and stale-event clearing on device teardown; `waylandTransferTest` covers plain clipboard state and real nonblocking pipe transfers; `waylandEventLoopTest` covers poll snapshots, deadline selection, readiness dispatch, and D-Bus watch and timeout state; `applicationTest` covers host behavior including tentative IME edits, captured paint damage, and scaled rendering coordinates.
