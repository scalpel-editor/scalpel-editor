...
The shell now has robust core-global, output, and seat lifetimes plus composed and repeating keyboard input, themed cursors, and asynchronous clipboard transfers. `WaylandWindow` owns the display, registry, required globals, xdg-toplevel, bound outputs, active seat devices, main and cursor surfaces, cursor theme, clipboard manager, and `wl_egl_window`; thin callbacks feed deterministic registry, output-membership, seat-capability, window, cursor, input, offer, and transfer state. `ApplicationEditor` owns Scintilla host state, drawing, invalidation, editor deadlines, idle work, and value-based clipboard requests and results. `main.cxx` moves resize, input, and clipboard values between the shell and editor and returns the retained cursor request, while the Wayland wait combines editor, key-repeat, and clipboard deadlines with display and transfer descriptors.
