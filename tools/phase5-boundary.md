...
Historical mentions in `MESSAGE_REMOVAL.md`, `ROADMAP.md`, `tools/phase5-boundary.md`, and commit messages are documentation of the old path, not live API.
...
`tools/check-no-message-layer.sh` greps the repository (excluding `seed/` and this freeze’s historical docs where noted) for forbidden live forms:
