...
8. **Complete the menu-bar workflow checks.** Add an end-to-end headless sequence covering keyboard open, pointer selection, edit-state changes, portal action, dirty Quit cancellation, and return to editor input. Update `ROADMAP.md` and ownership comments to describe the landed boundary, check changed production headers alone, and run `cmake --workflow --preset dev` because the finished slice crosses input, rendering, editor-host, workspace, and executable concerns. Perform a live Wayland smoke check for File/Edit opening, keyboard navigation, command execution, scaling, and dirty Quit; leave the sanitizer matrix for the Phase 8 gate.
