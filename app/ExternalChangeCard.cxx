...
// Overwrite, Reload, Save As, and Cancel are fixed behavior, not visual style.
constexpr int kButtonCount = 4;

int CardContentHeight(const UiStyle &style) noexcept {
	return style.cardPad + style.cardTitleHeight +
		style.externalChangeTitlePathGap + style.externalChangePathHeight +
		style.cardButtonTopGap + style.cardButtonHeight + style.cardPad;
}
...
ExternalChangeCardLayout LayoutExternalChangeCard(int width,
	int height) noexcept {
	const UiStyle &style = DefaultUiStyle();
	ExternalChangeCardLayout layout;
	if (width <= 0 || height <= 0) {
		return layout;
	}
	...
	int usedWidth = style.externalChangeCardMaxWidth;
	if (maxByClient > 0) {
		usedWidth = std::min(style.externalChangeCardMaxWidth, maxByClient);
	}
	if (usedWidth < style.externalChangeCardMinWidth &&
		width >= style.externalChangeCardMinWidth) {
		usedWidth = std::min(style.externalChangeCardMinWidth, width);
	}
	if (usedWidth > width) {
		usedWidth = width;
	}
	...
	const int naturalPathTop = cardTop + style.cardPad +
		style.cardTitleHeight + style.externalChangeTitlePathGap;
	const int naturalButtonTop = naturalPathTop +
		style.externalChangePathHeight + style.cardButtonTopGap;
	...
	if (pathTop + style.externalChangePathHeight > buttonTop) {
		const int textBottom =
			std::max(cardTop, buttonTop - style.cardButtonTopGap);
		pathTop = std::max(cardTop,
			textBottom - style.externalChangePathHeight);
		titleTop = std::max(cardTop,
			pathTop - style.externalChangeTitlePathGap - style.cardTitleHeight);
	}
	...
	layout.path = PRectangle::FromInts(
		...
		std::min(cardBottom, pathTop + style.externalChangePathHeight));

	const int totalGaps = style.externalChangeButtonGap * (kButtonCount - 1);
	...
	const auto makeButton = [&](int extraWidth) {
		...
		x += w + (innerWidth > totalGaps ? style.externalChangeButtonGap : 0);
		return rect;
	};
	layout.overwriteButton = makeButton(0);
	layout.reloadButton = makeButton(0);
	layout.saveAsButton = makeButton(0);
	layout.cancelButton = makeButton(leftover);

	return layout;
}

ExternalChangeCardHit HitTestExternalChangeCard(
	const ExternalChangeCardLayout &layout,
	Scintilla::Internal::Point point) noexcept {
	if (NonEmptyContains(layout.overwriteButton, point)) {
		return ExternalChangeCardHit::Overwrite;
	}
	if (NonEmptyContains(layout.reloadButton, point)) {
		return ExternalChangeCardHit::Reload;
	}
	if (NonEmptyContains(layout.saveAsButton, point)) {
		return ExternalChangeCardHit::SaveAs;
	}
	if (NonEmptyContains(layout.cancelButton, point)) {
		return ExternalChangeCardHit::Cancel;
	}
	return ExternalChangeCardHit::None;
}

int CycleExternalChangeCardFocus(int focusedIndex, int delta) noexcept {
...
ExternalChangeCardPainter::ExternalChangeCardPainter(const UiStyle &styleIn)
	: style(styleIn) {
...
void ExternalChangeCardPainter::Paint(Surface &surface,
	const ExternalChangeCardLayout &layout,
	...
	DrawButton(surface, layout.overwriteButton, bodyFont.get(), "Overwrite",
		focusedButtonIndex == 0, style);
	DrawButton(surface, layout.reloadButton, bodyFont.get(), "Reload",
		focusedButtonIndex == 1, style);
	DrawButton(surface, layout.saveAsButton, bodyFont.get(), "Save As",
		focusedButtonIndex == 2, style);
	DrawButton(surface, layout.cancelButton, bodyFont.get(), "Cancel",
		focusedButtonIndex == 3, style);
}
